...
void main() async {
//     test('Intraday', () async {
//     final timeSeries = new TimeSeries("LNNL7HMG2LSK7DP4");
//     //get the hourly time series for Microsoft with to-the-minute updates. You can change this.
//     JSONObject json = await timeSeries.getIntraday("IBM");
//     var data = json.getJSONMap();
//     // ["Time Series (60min)"];
...
//   test('Intraday', () async {
//     final technicalIndicator = new TechnicalIndicators("LNNL7HMG2LSK7DP4");
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  // get searchtext => this.searchtext;
  ...
  List<ChartSampleData> getChartData() {
    var data = Stocks();
    data.updateData();
    List<ChartSampleData> stockData = [];
    for (var i = 0; i < data.open.length; i++) {
      stockData.add(ChartSampleData(
          x: DateTime(data.date[i]),
          open: data.open[i],
          high: data.high[i],
          low: data.low[i],
          close: data.close[i]));
